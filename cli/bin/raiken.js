...
// Check if the compiled JS exists, if not, try to run from source (entry still `bin/raiken.js`)
const fs = require('fs');
...
if (!fs.existsSync(cliMainPath)) {
  ...
  console.error('Raiken CLI not built. Please run "npm run build" in the CLI directory.');
  process.exit(1);
}
