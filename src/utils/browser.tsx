...
const Browser = ({ url, theme, className }: props) => {
  ...
  const setUrlInHeader = () => {
    ...
    if (headerHeading && headerHeading?.innerHTML !== frameUrl) {
      headerHeading.innerHTML = frameUrl;
    }
  }
  ...
  const openInNewWindow = () => window.open(iframeWindow().location.href);

  const onUrlUpdate = (event: any) => setCurrentUrl(event.target.value);
  ...
  return (
    <div id={PARENT_CONTAINER} className={`${className}`}>
      <div className={`header ${themeClass}`}>
        ...
        <button onClick={forward} className='btn al-itm-c mr-rt-4 fs-18 centralize'>
          <svg width="18" height="18" viewBox="-8 -20 100 100" fill="none">
        ...
        <button onClick={reload} className='btn mr-rt-4 fs-24 centralize'>
        ...
        <div className='url-bar mr-rt-4 mr-lt-4'>
          <input onChange={(event) => onUrlUpdate(event)} id={URL_HEADING_ID} className="txt-elip browser_input" value={currentUrl} />
        </div>
        <button onClick={openInNewWindow} className='btn fs-22 mr-lt-4 centralize'>
